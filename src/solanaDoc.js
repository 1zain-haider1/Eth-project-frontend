...
const NETWORK_NAME = 'devnet';//'testnet'
export const buildSolanaTransferTransaction = async function ({
  from,
  to,
  token,
  amount,
  ...
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
...
export const deploySolanaToken = async function ({ from, to = from.publicKey, amount, fixedSupply, testnet = true, decimals = 9 }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new solanaWeb3.Connection(
    solanaWeb3.clusterApiUrl(network),
    'confirmed',
...
export const mintSolanaToken = async function ({ from, to = from.publicKey, token, amount, latestBlock, testnet = true }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new solanaWeb3.Connection(
    solanaWeb3.clusterApiUrl(network),
    'confirmed',
...
export const deploySolanaNFT = async function ({ from, maxSupply, uri, testnet = true }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
...
export const mintEdition = async function ({ masterEdition, from, testnet = true }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network)
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const buildSolanaTokenBurnTransaction = async function ({
  from,
  token,
  amount,
  latestBlock,
...
export const updateMetaplexMetadata = async function ({ token, from, uri, testnet = true }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network)
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const buildSolanaCustomProgramInteraction = async function ({
  from,
  programId,
  data,
  keys,
...
export const createTokenVault = async function ({ testnet, from }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const addTokenToVault = async function ({ testnet, vault, token, from }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
  const fromAccount = solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey))
...
export const activateVault = async function ({ testnet, vault, from }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const createStore = async function ({ testnet, from }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
...
export const createAuction = async function ({ testnet, from, vault, tickSize, endAuctionGap, endAuctionAt, gapTickSizePercentage, minumumPrice }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const initAuctionAuthority = async function ({ testnet, from, vault, store, auction }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const updateAuctionAuthority = async function ({ testnet, from, auction, auctionManager, latestBlock }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const fromAccount = solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey))
  let manualTransaction = new solanaWeb3.Transaction({
    recentBlockhash: latestBlock.toString(),
...
export const updateVaultAuthority = async function ({ testnet, from, vault, auctionManager, latestBlock }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const fromAccount = solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey))
  let manualTransaction = new solanaWeb3.Transaction({
    recentBlockhash: latestBlock.toString(),
...
export const whitelistCreators = async function ({ testnet, from, uri, store, latestBlock }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
...
export const validateAuction = async function ({ testnet, from, latestBlock, vault, nft, store, metadata, tokenStore, tokenTracker }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
  const storeId = toPublicKey(store);//await metaplex.programs.metaplex.Store.getPDA(wallet.publicKey);
  ...
  const whitelistedCreator =await metaplex.programs.metaplex.WhitelistedCreator.getPDA(toPublicKey(store), wallet.publicKey);// await metaplex.programs.metaplex.WhitelistedCreator.getPDA(toPublicKey(store), wallet.publicKey)
...
export const startAuction = async function ({ testnet, from, store, auction, auctionManager }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const placeBid = async function ({ testnet, from, auction, amount }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const cancelBid = async function ({ testnet, from, auction, bidderPotToken }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const finishAuction = async function ({ testnet, from, auction, store, auctionManager, vault, auctionManagerAuthority }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const redeemAuction = async function ({ testnet, from, auction, store }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const claimWinnings = async function ({ testnet, from, auction, store, bidderPotToken }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const listAuctions = async function ({ testnet, from, authority }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
  const wallet = new metaplex.NodeWallet(solanaWeb3.Keypair.fromSecretKey(bs58.decode(from.privateKey)))
...
export const inspectAuction = async function ({ testnet, auction }) {
  const network = testnet ? NETWORK_NAME : 'mainnet-beta'
  const connection = new metaplex.Connection(network, "confirmed")
